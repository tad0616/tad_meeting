    ...
    <tbody id="tad_meeting_sort">
      <{foreach from=$all_content item=data}>
        <tr id="tr_<{$data.tad_meeting_sn}>">
          ...
          <{if $isAdmin}>
            <td>
              <a href="javascript:delete_tad_meeting_func(<{$data.tad_meeting_sn}>);" class="btn btn-xs btn-danger"><{$smarty.const._TAD_DEL}></a>
              <a href="<{$xoops_url}>/modules/tad_meeting/index.php?op=tad_meeting_form&tad_meeting_sn=<{$data.tad_meeting_sn}>" class="btn btn-xs btn-warning"><{$smarty.const._TAD_EDIT}></a>
            </td>
          <{/if}>
        </tr>
      <{/foreach}>
