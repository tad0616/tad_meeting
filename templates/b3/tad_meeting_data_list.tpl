          ...
          <div style="line-height: 1.8; padding: 10px 0px 0px 48px; text-align: justify;">
            ...
            <{if $isAdmin or $now_uid==$data.tad_meeting_data_uid}>
              <div class="text-right">
                ...
                <a href="javascript:delete_tad_meeting_data_func(<{$data.tad_meeting_data_sn}>);" class="btn btn-xs btn-danger"><{$smarty.const._TAD_DEL}></a>
                <a href="<{$xoops_url}>/modules/tad_meeting/index.php?tad_meeting_sn=<{$tad_meeting_sn}>&tad_meeting_data_sn=<{$data.tad_meeting_data_sn}>#tad_meeting_data_form" class="btn btn-xs btn-warning"><{$smarty.const._TAD_EDIT}></a>
                <{if $isAdmin or $sort_report}>
